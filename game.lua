...
function Game:draw()
    ...
    -- Draw Test Win Button
    local winFont = love.graphics.newFont(12)
    love.graphics.setFont(winFont)
    love.graphics.setColor(0.2, 0.6, 1)
    love.graphics.rectangle("fill", self.testWinButton.x, self.testWinButton.y, self.testWinButton.width, self.testWinButton.height, 10)
    love.graphics.setColor(1, 1, 1)
    love.graphics.printf("I give up button", self.testWinButton.x, self.testWinButton.y + 10, self.testWinButton.width, "center")
    ...
    if #self.stock > 0 then
        ...
        love.graphics.draw(backImage, 100, 50, 0,1.5)
    end
...
function Game:mousereleased(x, y, button)
    ...
    -- Handle test win button click
      if x >= self.testWinButton.x and x <= self.testWinButton.x + self.testWinButton.width and
         y >= self.testWinButton.y and y <= self.testWinButton.y + self.testWinButton.height then
            self:forceWin()
            return
        end
...
function Game:forceWin()
    local suits = { "card_spades_", "card_hearts_", "card_clubs_", "card_diamonds_" }
    local ranks = { "A", "02", "03", "04", "05", "06", "07", "08", "09", "10", "J", "Q", "K" }

    -- Clear all foundations first
    for _, foundation in ipairs(self.foundations) do
        foundation.cards = {}
    end

    -- Place each card in order in the right foundation
    for s, suit in ipairs(suits) do
        for _, rank in ipairs(ranks) do
            local card = Card:new(suit, rank, true)
            table.insert(self.foundations[s].cards, card)
        end
    end
    self.hasWon = true
end
